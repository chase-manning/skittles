...
- **Familiar types**: Use `number` for amounts and counters, `string` for text, `boolean` for flags, `address` for wallet addresses, `bytes` for raw data, `Record<K,V>` for key-value storage, arrays, type aliases for custom structures, interfaces, and enums

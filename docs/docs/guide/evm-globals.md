...
| `msg.value`  | `number`  | The amount of ETH sent with the call (wrapped in `int256(...)` for `number` compatibility) |
...
| Property          | Type      | Description                                                     |
| ----------------- | --------- | --------------------------------------------------------------- |
| `block.timestamp` | `number`  | The timestamp of the current block (in seconds since Unix epoch). Wrapped in `int256(...)` for `number` compatibility. |
| `block.number`    | `number`  | The current block number. Wrapped in `int256(...)` for `number` compatibility. |
| `block.chainid`   | `number`  | The chain ID (1 for Ethereum mainnet, etc.). Wrapped in `int256(...)` for `number` compatibility. |
...
| `tx.gasprice` | `number`  | The gas price of the transaction. Wrapped in `int256(...)` for `number` compatibility. |

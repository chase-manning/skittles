...
function makeFunction(name: string, body: Statement[], params: string[] = []): SkittlesFunction {
  return {
    name,
    parameters: params.map((p) => ({ name: p, type: { kind: SkittlesTypeKind.Int256 } })),
    returnType: null,
    visibility: "public",
...
describe("unreachable code detection", () => {
  it("should not warn for a function with no issues", () => {
    const fn = makeFunction("test", [
      { kind: "variable-declaration", name: "x", type: { kind: SkittlesTypeKind.Int256 }, initializer: { kind: "number-literal", value: "1" } },
  ...
  it("should warn about code after break", () => {
    const fn = makeFunction("test", [
      {
        kind: "for",
        initializer: { kind: "variable-declaration", name: "i", type: { kind: SkittlesTypeKind.Int256 }, initializer: { kind: "number-literal", value: "0" } },
...
describe("unused variable detection", () => {
  ...
  it("should detect usage in for loop body", () => {
    const fn = makeFunction("test", [
      { kind: "variable-declaration", name: "total", type: { kind: SkittlesTypeKind.Int256 }, initializer: { kind: "number-literal", value: "0" } },
      {
        kind: "for",
        initializer: { kind: "variable-declaration", name: "i", type: { kind: SkittlesTypeKind.Int256 }, initializer: { kind: "number-literal", value: "0" } },
        condition: { kind: "binary", operator: "<", left: { kind: "identifier", name: "i" }, right: { kind: "number-literal", value: "10" } },
  ...
  it("should warn about multiple unused variables", () => {
    const fn = makeFunction("test", [
      { kind: "variable-declaration", name: "a", type: { kind: SkittlesTypeKind.Int256 }, initializer: { kind: "number-literal", value: "1" } },
      { kind: "variable-declaration", name: "b", type: { kind: SkittlesTypeKind.Int256 }, initializer: { kind: "number-literal", value: "2" } },
      { kind: "return", value: { kind: "number-literal", value: "0" } },
    ]);
    const warnings = analyzeFunction(fn, "MyContract");
  ...
  it("should not flag a variable used only in an assignment target", () => {
    ...
    const fn = makeFunction("test", [
      { kind: "variable-declaration", name: "x", type: { kind: SkittlesTypeKind.Int256 }, initializer: { kind: "number-literal", value: "0" } },
      {
        kind: "expression",
        expression: {
          kind: "assignment",
...
describe("combined unreachable code and unused variables", () => {
  it("should detect both unreachable code and unused variables", () => {
    const fn = makeFunction("test", [
      { kind: "variable-declaration", name: "unused", type: { kind: SkittlesTypeKind.Int256 }, initializer: { kind: "number-literal", value: "1" } },
      { kind: "return", value: { kind: "number-literal", value: "0" } },
      { kind: "expression", expression: { kind: "identifier", name: "x" } },
    ]);
    const warnings = analyzeFunction(fn, "MyContract");

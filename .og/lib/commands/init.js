...
const updateGitignore = () => {
    ...
    if (exists(gitignorePath)) {
        const currentContent = fs_1.default.readFileSync(gitignorePath, "utf8");
        ...
        fs_1.default.writeFileSync(gitignorePath, entries);
    }
};
/**
...
const updatePackageJson = () => {
    ...
    try {
        ...
        throw new Error(`Failed to parse package.json: ${(error === null || error === void 0 ? void 0 : error.message) || "Unknown error"}`);
    }
 ...
 */
const createExampleContracts = (force = false) => {
    ensureContractsDirectory();
    const ierc20Path = path_1.default.join(process.cwd(), "contracts", "ierc20.ts");
    const erc20Path = path_1.default.join(process.cwd(), "contracts", "erc20.ts");
    ...
    fs_1.default.writeFileSync(ierc20Path, IERC20_TEMPLATE);
    fs_1.default.writeFileSync(erc20Path, ERC20_TEMPLATE);
};
/**
 * Initializes a new Skittles project.
 */
const initSkittles = (options = {}) => {
    const spinner = (0, ora_1.default)("Initializing Skittles project").start();
    try {
        // Check if config already exists
        const configPath = path_1.default.join(process.cwd(), "skittles.config.ts");
        ...
        spinner.fail(`Failed to initialize project: ${(error === null || error === void 0 ? void 0 : error.message) || "Unknown error"}`);
        throw error;
    }
};
exports.initSkittles = initSkittles;
